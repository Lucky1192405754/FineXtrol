...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automatically scan .log files in the specified directory, extract average evaluation metrics, and print them by row."
    )
    parser.add_argument(
        '--log_dir', 
        type=str, 
    ...
    log_files = sorted([f for f in os.listdir(args.log_dir) if f.endswith('.log')])

    if not log_files:
        print(f"No matching log files were found in directory '{args.log_dir}' .")
        return
