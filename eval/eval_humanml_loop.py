...
if __name__ == '__main__':
    ...
    body_parts_to_evaluate = args.eval_parts or [args.eval_part]
    mask_ratios_to_evaluate = [args.mask_ratio]

    for part in body_parts_to_evaluate:
        for ratio in mask_ratios_to_evaluate:
            print(f"\n====================================================================")
            print(f"STARTING EVALUATION: Body Part = [{part}], Mask Ratio = [{int(ratio*100)}%]")
            print(f"====================================================================")
            ...
            log_file += f'_part_{part.replace(" ", "")}_ratio_{int(ratio*100)}'
            log_file += '.log'
            ...
            gt_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='gt', 
                                           control_joint=args.control_joint, density=args.density, eval_part=part, mask_ratio=ratio)
            ...
            print(f"--- FINISHED EVALUATION: Body Part = [{part}], Mask Ratio = [{int(ratio*100)}%] ---")
    
    print("\nAll evaluation cycles completed!")