...
if __name__ == '__main__':
    ...
    log_file += f'_part_{args.eval_part.replace(" ", "")}'
    log_file += f'_mask_{int(args.mask_ratio * 100)}'
    log_file += '.log'
    ...
    if args.eval_mode == 'finextrol':
        ...
        mm_num_times = 0
        diversity_times = 300
        replication_times = 3  # about 3 Hrs * rep_times
    else:
        raise ValueError()
    ...
    gen_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='eval', control_joint=args.control_joint, density=args.density, eval_part=args.eval_part, mask_ratio=args.mask_ratio)
