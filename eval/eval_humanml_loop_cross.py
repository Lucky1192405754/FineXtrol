...
if __name__ == '__main__':
    ...
    fixseed(args.seed) #
    args.batch_size = 32 #  This must be 32! Don't change it! otherwise it will cause a bug in R precision calc! [cite: 45-46]
    
    ALL_BODY_PARTS = ["head", "body", "left hand", "right hand", "left leg", "right leg"]
    K_VALUES_TO_EVALUATE = args.cross_k
    mask_ratios_to_evaluate = [args.mask_ratio]
    
    for k in K_VALUES_TO_EVALUATE:
        print(f"\n====================================================================")
        print(f"STARTING 'CROSS' EVALUATION FOR K = {k} (C(6, {k}))")
        
        for part_combination in itertools.combinations(ALL_BODY_PARTS, k):
            
            part_list = list(part_combination) 
            
            for ratio in mask_ratios_to_evaluate: 
                print(f"\n--------------------------------------------------------------------")
                print(f"STARTING EVAL: K={k}, Parts={part_list}, Mask Ratio=[{int(ratio*100)}%]")
                print(f"--------------------------------------------------------------------")

                name = os.path.basename(os.path.dirname(args.model_path)) 
                niter = os.path.basename(args.model_path).replace('model', '').replace('.pt', '') 
                
                log_file = os.path.join(os.path.dirname(args.model_path), f'eval_humanml_{name}_{niter}') 
                if args.guidance_param != 1.: 
                    log_file += f'_gscale{args.guidance_param}'
                log_file += f'_{args.eval_mode}' 
                
                part_str = "_".join(p.replace(" ", "") for p in part_list)
                log_file += f'_k{k}_parts_{part_str}_ratio_{int(ratio*100)}'
                log_file += '.log' 

                if os.path.exists(log_file):
                    print(f"Log file [{os.path.basename(log_file)}] already exists.")
                ...
                gt_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='gt', 
                                               control_joint=args.control_joint, density=args.density, 
                                               eval_part=part_list, mask_ratio=ratio) 
                ...
                print(f"--- FINISHED EVALUATION: K={k}, Parts={part_list}, Ratio=[{int(ratio*100)}%] ---") 
    
    print("\nAll 'cross' evaluation cycles completed!") 