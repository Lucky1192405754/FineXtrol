...
# FINAL VERSION - Re-integrated detailed_text functionality.
# ------------------------------------------------------------------------------------
...
#    - Now supports optional --detailed_text for fine-grained control.
...
#
# To Run (coarse-grained only):
# python smpl_visualization.py --model_path ./path/to/model.pt --prompt "a person is walking"
#
# To Run (with fine-grained control):
# python smpl_visualization.py --model_path ./path/to/model.pt --prompt "a person moves" --detailed_text "..."
# ------------------------------------------------------------------------------------
...
CAMERA_HEIGHT = 1.5
CAMERA_DISTANCE = 3.5
MINIMUM_CAMERA_Y = 0.5

# =================================================================================================
# [STEP 3 LOGIC]: OBJ SEQUENCE RENDERING (No changes)
# =================================================================================================
large_scale = 1.8   #1.8
height = 0.2        #0.1
distance = 3.0      #3.0
def look_at(camera_position, target, up=np.array([0, 1, 0])):
    ...
    return view_matrix
def load_obj_as_mesh(obj_path, new_color=None):
...
# [fix002]
def create_ground_plane(width=4.0, depth=4.0, y=0.0, color=[0.8, 0.8, 0.8, 1.0], center=np.array([0, 0, 0])):
    """
    Modify this function to adjust the floor appearance, including size and color.
    """
    vertices = np.array([
    ...
    # [fix003]
    # material = pyrender.material.UnlitMaterial(baseColorFactor=color)
    material = pyrender.MetallicRoughnessMaterial(
...
def render_frame(scene, model_center):
    ...
    # [fix004]
    # offset = np.array([0, CAMERA_HEIGHT, CAMERA_DISTANCE])
    camera_position = model_center + offset

    camera_position[1] = max(camera_position[1], MINIMUM_CAMERA_Y)
...
def step3_render_final_video(obj_dir, output_dir, motion_name, fps=20):
    ...
    START_COLOR = np.array([0.6, 0.8, 1.0])
    END_COLOR = np.array([0.0, 0.1, 0.5])
    ...
    for frame_idx, obj_file in enumerate(tqdm(obj_files, desc="Rendering Frames")):
        ...
        current_color_rgb = START_COLOR * (1 - progress) + END_COLOR * progress
        
        current_color_rgba = np.append(current_color_rgb, 1.0)

        mesh, centroid = load_obj_as_mesh(obj_path, new_color=current_color_rgba)
...
# =================================================================================================
# [STEP 1 LOGIC]: TEXT TO SKELETON GENERATION (Updated to include detailed_text)
# =================================================================================================

def step1_generate_skeleton(args, motion_name):
    """
    Generates a BATCH of skeleton motions from a text prompt, with optional fine-grained control.
    """
