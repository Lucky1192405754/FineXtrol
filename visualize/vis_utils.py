...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        if 'detailed_text' in self.motions:
            print(f"Length of detailed_text: {len(self.motions['detailed_text'])}")
            print(f"Value of absl_idx: {self.absl_idx}")
            self.detailed_text = self.motions['detailed_text'][self.absl_idx]
            # sys.exit()
        else:
