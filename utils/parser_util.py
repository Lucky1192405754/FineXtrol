...
def parse_and_load_from_model(parser):
    ...
    for a in args_to_overwrite:
        if a in model_args.keys():
            if a not in ['control_joint', 'density', 'mask_ratio', 'eval_part', 'eval_parts', 'cross_k']:
...
def add_data_options(parser):
    ...
    group.add_argument("--dataset", default='detailed_text', choices=['humanml', 'kit', 'humanact12', 'uestc', 'detailed_text'], type=str,
    ...
    group.add_argument("--density", default=100, type=int,
                       help="density")
    group.add_argument("--mask_ratio", default=0.5, type=float,
                       help="Fine-grained text mask ratio.")
    group.add_argument("--eval_part", default='left leg', type=str,
                       choices=['head', 'body', 'left hand', 'right hand', 'left leg', 'right leg'],
                       help="Body part used for single-part FineXtrol evaluation.")
    group.add_argument("--eval_parts", nargs='+', default=None,
                       choices=['head', 'body', 'left hand', 'right hand', 'left leg', 'right leg'],
                       help="Body parts to evaluate in eval_humanml_loop.py. Defaults to --eval_part.")
    group.add_argument("--cross_k", nargs='+', type=int, default=[2],
                       help="Combination sizes for cross-body-part evaluation.")
...
def add_training_options(parser):
    ...
    # group.add_argument("--mask_ratio", default=0.5, type=float,
    #                    help="Density of fine-grained textual control signals")
...
def add_generate_options(parser):
    ...
    group.add_argument("--text_prompt", default='A person walks in a clockwise circle.', type=str,
                       help="A coarse text prompt to be generated. If empty, will take text prompts from dataset.")
    group.add_argument("--detailed_text_prompt", default='<SEP> <Motionless> <SEP> Move your left leg back slightly. <SEP> <Motionless> <SEP> Move your left leg forward.', type=str,
                       help="Fine-grained control text with <SEP>, <Motionless>, and <Mask> tokens.")
...
def add_evaluation_options(parser):
    ...
    group.add_argument("--model_path", required=True, type=str,
                       help="Path to model####.pt file to be sampled.")
    group.add_argument("--eval_mode", default='finextrol', choices=['finextrol'], type=str,
                       help="")
    group.add_argument("--guidance_param", default=2.5, type=float,
                       help="For classifier-free sampling - specifies the s parameter, as defined in the paper.")
